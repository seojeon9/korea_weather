...
def get_region_id(region: str) -> str:
    """지역 이름을 입력받아 지역 코드를 반환합니다.
...
def get_middle_term_regid(region: str) -> str:
    """중기예보 권역별 regId를 지역명으로부터 추론하여 반환합니다."""
    region_map = {
        '11B00000': ['서울', '인천', '경기도'],
        '11D10000': ['강원도영서', '춘천', '원주', '강원영서'],
        '11D20000': ['강원도영동', '강릉', '동해', '속초', '강원영동'],
        '11C20000': ['대전', '세종', '충청남도', '충남'],
        '11C10000': ['충청북도', '충북'],
        '11F20000': ['광주', '전라남도', '전남'],
        '11F10000': ['전북자치도', '전라북도', '전북'],
        '11H10000': ['대구', '경상북도', '경북'],
        '11H20000': ['부산', '울산', '경상남도', '경남'],
        '11G00000': ['제주도', '제주'],
    }
    region = region.strip()
    for regid, names in region_map.items():
        for name in names:
            if name in region:
                return regid
    return None

async def get_middle_term_forecast_from_api(region: str) -> str:
    """특정 지역의 중기 육상 예보 (3일~10일) 정보를 조회합니다.

    Args:
        region (str): 지역 이름

    Returns:
        str: 중기 육상 예보 (3일~10일) 정보 문자열
    """

    region_id = get_middle_term_regid(region)
    if not region_id:
        return f"지원하지 않는 지역입니다: {region}"
    api_key = urllib.parse.unquote(KOREA_WEATHER_API_KEY)
    now = datetime.now()
    base_times = ['0600', '1800']
    current_hour = int(now.strftime('%H'))
    available_time = None
    for bt in base_times:
        bt_hour = int(bt[:2])
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            item = items[0]
            result = []
            result.append(f"[중기예보: {region}]")
            for day in range(4, 11):
                if day <= 7:
                    am_rain = item.get(f'rnSt{day}Am')
                    pm_rain = item.get(f'rnSt{day}Pm')
                    am_wf = item.get(f'wf{day}Am')
                    pm_wf = item.get(f'wf{day}Pm')
                    if am_rain is not None or pm_rain is not None or am_wf is not None or pm_wf is not None:
                        result.append(f"{day}일 후 오전: {am_wf or '-'} (강수확률 {am_rain if am_rain is not None else '-'}%) / 오후: {pm_wf or '-'} (강수확률 {pm_rain if pm_rain is not None else '-'}%)")
                else:
                    rain = item.get(f'rnSt{day}')
                    wf = item.get(f'wf{day}')
                    if rain is not None or wf is not None:
                        result.append(f"{day}일 후: {wf or '-'} (강수확률 {rain if rain is not None else '-'}%)")
            return '\n'.join(result)
    except Exception as e:
        return f"예보 파싱 오류: {e}"

async def get_dust_week_forecast_from_api() -> str:
    """미세먼지 주간예보를 가져옵니다.
...
@mcp.tool()
async def get_middle_term_forecast(region: str) -> str:
    """특정 지역의 중기 육상 예보 (3일~10일) 정보를 가져옵니다.
...
@mcp.tool()
async def get_week_dust_forecast() -> str:
    """미세먼지 주간예보를 가져옵니다.
    ...
    """
    response = await get_dust_week_forecast_from_api()
    return response
